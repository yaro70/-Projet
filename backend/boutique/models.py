...
class Commande(models.Model):
    ...
    qr_code = models.TextField(blank=True, null=True)  # Stockage du QR code en base64
...
📅 Date de récupération: {self.date_livraison.strftime('%d/%m/%Y à %H:%M')}
...
J'ai effectué le dépôt et je souhaite envoyer la capture d'écran."""
        ...
        try:
            ...
            numero_patron = parametres.numero_patron if parametres else "2250123456789"
        except:
            numero_patron = "2250123456789"
        
        return f"https://wa.me/{numero_patron}?text={encoded_message}"
...
class ParametresLivraison(models.Model):
    ...
    numero_patron = models.CharField(max_length=20, default="2250123456789", help_text="Numéro de téléphone du patron pour les dépôts")
    
    def __str__(self):
        return f"Prix livraison: {self.prix_livraison}€ - Tel: {self.numero_patron}"
